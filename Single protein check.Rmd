...
#remove rows with na values in all df
```{r}
tidy_master_ratio <- tidy_master_ratio %>%
  select(-c("Protein names","Fasta headers","ORF")) %>%
  melt() %>%
  group_by(`Protein IDs`,`Gene names`,genotype, nutrient, samplingTime, replicate) %>%
...
tmp<-tidy_master_ratio %>% spread(key = variable, value = value)
  
write_csv(tmp, "./tables/master_tidy_protein.csv")
```
