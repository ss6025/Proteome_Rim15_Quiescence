...
```{r}
IGO1 <- proteome %>% filter(`Gene names` == "IGO1") %>%
...
ggsave(
   filename = "./plots/individual_plots_protein.pdf", 
   plot = marrangeGrob(plot_list, nrow=4, ncol=2), 
   width = 15, height = 10
)

#create pdf where each page is a separate plot.
pdf("./plots/individual_plots_protein.pdf")
for (i in 1:length(protein_list)) {
  par(mfrow = c(2, 4))
  print(plot_list[[i]])
}
