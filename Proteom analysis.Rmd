...
Save ratios, normalized ratios and intensities into different data frames in tidy version
```{r}
...
###Make a table containing all informations: intensity, normalized_intensity, ratios, normalized_ratios
```{r}
#read in the normalized ratio and intensity table from single protein check.Rmd
proteome <- read_csv("./tables/proteinTable.csv")
tidy_master_ratio<- left_join(ratio_tidy_re, proteome %>% rename (normalized_ratio = value))
```
